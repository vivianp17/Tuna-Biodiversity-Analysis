...
Summary_Table_plot <- plot_ly(
  ...
  type = "bar",
  orientation = "h",
  text = ~paste("Species: ", species_name, "<br>Samples: ", num_samples),
  hoverinfo = "text"
) %>%
  layout(
...
# Filter Data
#Function for filtering data based on nucleotides in order to only maintain the highest quality data
filter_sequences <- function(df, min_seq_length = 500, nucleotide_column = "nucleotides") {
  df %>%
    mutate(
      seq_length = nchar(!!sym(nucleotide_column)),
      !!sym(nucleotide_column) := toupper(!!sym(nucleotide_column))  # Ensure uppercase nucleotides
    ) %>%
    filter(
      seq_length >= min_seq_length,
      !grepl("[^ATCG]", !!sym(nucleotide_column))  # Remove sequences with ambiguous nucleotides
    ) %>%
    select(-seq_length)
}
...
df_tuna_BOLD.sub <- filter_sequences(df_tuna_BOLD.sub, min_seq_length = 500, nucleotide_column = "nucleotides")
...
# Visualize filtered summarized data by species and country
filtered_Summary_Table_plot <- plot_ly(
